...
const loadBannedCountries = async () => {
  try {
    const bannedCountries = (await axios.get(API_URL + 'countries/forbidden')).data;
    bannedCountries.forEach((countryCode: string) => BANNED_COUNTRIES.add(countryCode));
...
const loadTokens = async () => {
  ...
  for (const [id, token] of (Object.entries(tokens) as any)) {
    if (!token.network || SUPPORTED_NETWORK_MAPPING[token.network] === undefined) continue;
    ...
    cryptoData[token.symbol].networks.push(SUPPORTED_NETWORK_MAPPING[token.network]);
  }
...
export const getPQuote = async(network: string, cryptoCurrency: string, fiatCurrency: string, amount: number,) => {
  const internalId = tokenToInteralIdMapping[cryptoCurrency];
  if (!internalId) throw new Error('Token not supported');
  if (!SUPPORTED_FIAT_CURRENCIES.has(fiatCurrency)) throw new Error('Fiat currency not supported');
  const reverseNetworkMapping = Object.fromEntries(Object.entries(SUPPORTED_NETWORK_MAPPING).map(([k, v]) => [v, k]))[network];
  ...
  try {
    ...
    return [
      ...
      paymentMethod: 'card',
      fee: parseFloat(resp.data.fees.networkFee || 0) + parseFloat(resp.data.fees?.fixFee || 0)
    }
  ]
  } catch (err:any) {
...
export const getMtPellerinUrl = (cryptoCurrency: string, fiatCurrency: string, network: string, amount: number, address: string) => {
  const reverseNetworkMapping = Object.fromEntries(Object.entries(SUPPORTED_NETWORK_MAPPING).map(([k, v]) => [v, k]))[network];
  if (!reverseNetworkMapping) throw new Error('Network not supported');
